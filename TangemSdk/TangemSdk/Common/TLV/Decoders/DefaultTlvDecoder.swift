...
protocol TlvDecoder: TlvLogging {
    func getTlv(for tag: TlvTag) -> Tlv?
    func decodeOptional<T>(_ tag: TlvTag) throws -> T?
    func decode<T>(_ tag: TlvTag) throws -> T
}
...
extension TlvDecoder {
    ...
    public func decodeOptional<T>(_ tag: TlvTag) throws -> T? {
        do {
            let decoded: T = try innerDecode(tag, tagValue: getTlv(for: tag)?.value, asOptional: true)
            logTlv(tag, decoded)
            return decoded
    ...
    public func decode<T>(_ tag: TlvTag) throws -> T {
        do {
            let decoded: T = try innerDecode(tag, tagValue: getTlv(for: tag)?.value, asOptional: false)
            logTlv(tag, decoded)
            return decoded
        } catch {
            Log.error(error)
    ...
    func innerDecode<T>(_ tag: TlvTag, tagValue: Data?, asOptional: Bool) throws -> T {
        ...
        case .interactionMode:
            ...
            if let mode = IssuerExtraDataMode(rawValue: byte)  {
                ...
                return mode as! T
            } else if let mode = ReadMode(rawValue: byte) {
                try typeCheck(ReadMode.self, T.self, for: tag)
                return mode as! T
            } else {
    ...
    func typeCheck<T1, T2>(_ expected: T1.Type, _ current: T2.Type, for tag: TlvTag) throws {
    ...
    func logTlv<T>(_ tag: TlvTag, _ value: T) {
        let tlvItem = getTlv(for: tag) ?? Tlv(tag, value: Data(hexString: "00")) //dummy tlv for boolean values
...
public final class DefaultTlvDecoder: TlvDecoder {
    let tlv: [TlvTag: Tlv]
