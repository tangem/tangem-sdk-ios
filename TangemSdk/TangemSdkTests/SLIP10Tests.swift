...
class SLIP10FWTests: FWTestCase {
        ...
        sdk.startSession(with: iw) { result in
            do {
                ...
                case .success(let response):
                    ...
                    self.printEquals("0266874dc6ade47b3ecd096745ca09bcd29638dd52c2c12117b11ed3e458cfa9e8".uppercased(), publicKey.hexString)
    ...
    func testVector2Secp256r1() throws {
        let sdk = TangemSdk()
        sdk.config.defaultDerivationPaths = [.secp256r1: [
            try DerivationPath(rawPath: "m/0"),
        ...
        sdk.startSession(with: iw) { result in
            do {
                ...
                case .success(let response):
                    ...
                    // Chain m
                    let publicKey = try XCTUnwrap(wallet.publicKey)
                    self.printEquals("02c9e16154474b3ed5b38218bb0463e008f89ee03e62d22fdcc8014beab25b48fa".uppercased(), publicKey.hexString)
        ...
        sdk.startSession(with: iw) { result in
            do {
                ...
                case .success(let response):
                    ...
                    // Chain m/28578H
                    let derived = try XCTUnwrap(wallet.derivedKeys[try DerivationPath(rawPath: "m/28578'")])
                    ...
                    let derived1 = try XCTUnwrap(wallet.derivedKeys[try DerivationPath(rawPath: "m/28578'/33941")])
    ...
    func testVector1Ed5519Slip0010() throws {
        let sdk = TangemSdk()
        ...
        sdk.startSession(with: iw) { result in
            do {
                ...
                case .success(let response):
                    ...
                    // Chain m
                    let publicKey = try XCTUnwrap(wallet.publicKey)
                    self.printEquals("00a4b2856bfec510abab89753fac1ac0e1112364e7d250545963f135f2a33188ed".dropFirst(2).uppercased(), publicKey.hexString)
    ...
    func testVector2Ed5519Slip0010() throws {
        let sdk = TangemSdk()
        sdk.config.defaultDerivationPaths = [.ed25519_slip0010: [
            try DerivationPath(rawPath: "m/0'"),
            try DerivationPath(rawPath: "m/0'/2147483647'"),
        ...
        sdk.startSession(with: iw) { result in
            do {
                ...
                case .success(let response):
                    ...
                    // Chain m
                    let publicKey = try XCTUnwrap(wallet.publicKey)
                    self.printEquals("008fe9693f8fa62a4305a140b9764c5ee01e455963744fe18204b4fb948249308a".dropFirst(2).uppercased(), publicKey.hexString)
