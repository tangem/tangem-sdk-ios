...
class TlvTests: XCTestCase {
    ...
    func testTlvDeserialization() throws {
        ...
        let deserialized = try XCTUnwrap(Tlv.deserialize(testData))
    ...
    func testDecoding() throws {
        ...
        let tlv = try XCTUnwrap(Tlv.deserialize(testData))
        ...
        let someMethods: SigningMethod = try XCTUnwrap(Data(hexString: "070195").decodeTlv(tag: .signingMethod))
        ...
        //get cardData
        let cardData = try XCTUnwrap(tlv.value(for: .cardData))
        let cardDataTlv = try XCTUnwrap(Tlv.deserialize(cardData))
        ...
        //test productMask
        let productMask: ProductMask = try XCTUnwrap(Data(hexString: "8A0102").decodeTlv(tag: .productMask))
        ...
        //test byte
        let testByte: Data = try XCTUnwrap(Data(hexString: "510109").decodeTlv(tag: .transactionOutHashSize))
        XCTAssertEqual(testByte.toInt(), Int(9))
        
        //test byte2
        let testBytes2: Data = try XCTUnwrap(Data(hexString: "51020384").decodeTlv(tag: .transactionOutHashSize))
    ...
    func testFileSettings() throws {
        let byte = UInt8(0x11)
        let settings = try XCTUnwrap(try FileSettings(Data(byte)))
